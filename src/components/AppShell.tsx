...
export function AppShell({
  ...
  menu?: MenuItem[];
  /** Label kecil di atas judul, misal "Sopo Harimotting" atau "Sopo Harimotting · Admin" */
  label?: string;
}) {
  return (
    <div
      className={
        menu && menu.length > 0 ? "min-h-screen pb-28" : "min-h-screen pb-8"
      }
    >
            ...
            {menu.map((m) => (
              <Link
                ...
                className={`flex flex-col items-center gap-0.5 text-center font-bold text-muted-foreground ${
                  menu.length > 4
                    ? "px-0.5 py-2 text-[11px] leading-tight"
                    : "py-3 text-base"
                }`}
