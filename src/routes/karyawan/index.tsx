...
export const Route = createFileRoute("/karyawan/")({
  head: () => ({ meta: [{ title: "Dashboard Absensi — Sopo Harimotting" }] }),
...
function DashboardKaryawan() {
  ...
  const absenMutation = useMutation({
    mutationFn: () =>
      absenDenganKode({ data: { kode: sesi!.kode, foto: foto! } }),
    onSuccess: () => {
        ...
        {sesiTerbuka ? (
          <>
            ...
            <p className="mt-1 text-sm opacity-85">
              Absen masuk pukul {jam(sesiTerbuka.masuk)}
            </p>
          </>
        ) : (
          <>
            ...
            <p className="mt-1 text-sm opacity-85">
              {riwayat[0]
                ? `Terakhir: ${jam(riwayat[0].masuk)} - ${riwayat[0].keluar ? jam(riwayat[0].keluar) : "-"}`
                : "Belum pernah absen"}
            </p>
          </>
        )}
      ...
      <div className="mt-8">
        ...
        {kelompok.length === 0 ? (
          ...
          <div className="space-y-6">
            {kelompok.map((k) => (
              <div key={k.kunci}>
                <p className="mb-2 px-1 font-display text-base font-black">
                  {k.label}
                </p>
                <div className="kartu-farm divide-y-2 divide-dashed divide-border overflow-hidden">
                  {k.list.map((a) => (
                    <div key={a.kode} className="flex items-center gap-3 p-4">
                      ...
                      <p className="shrink-0 text-right text-xs font-bold text-primary">
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Ambil foto dulu sebagai bukti kamu di lokasi.
            </p>
            ...
            {foto ? (
              <img
                src={foto}
                alt="Preview foto"
                className="mt-4 w-full rounded-2xl border-4 border-wood object-cover"
              />
            ) : (
              <button
              ...
              >
                ...
                <span className="mt-2 block font-black text-secondary-foreground">
                  Ambil / Upload Foto
                </span>
              </button>
            )}
