...
export const Route = createFileRoute("/admin/verifikasi")({
  head: () => ({
    meta: [{ title: "Verifikasi Pesanan — Sopo Harimotting" }],
  }),
            ...
            <div key={p.kode} className="kartu-farm p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-display text-xl font-black">
                    {p.namaPembeli || "(tanpa nama)"}
                  </p>
                  <p className="text-sm font-bold text-muted-foreground">
                    {p.kode} · {LABEL[p.kategori]} × {p.jumlah} ·{" "}
                    {tanggalJam(p.dibuatPada)}
                  </p>
                </div>
              ...
              <div className="mt-4 flex gap-2">
                <button
                  ...
                  disabled={
                    setujuiMutation.isPending || tolakMutation.isPending
                  }
