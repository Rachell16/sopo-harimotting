...
import { ambilKaryawan, buatKaryawan, hapusKaryawan, updateKaryawan } from "@/lib/karyawan.server";
import type { Karyawan } from "@/lib/karyawan";

export const Route = createFileRoute("/manager/karyawan")({
  head: () => ({ meta: [{ title: "Karyawan — Sopo Harimoting" }] }),
...
function KaryawanPage() {
  ...
  const simpanMutation = useMutation({
    mutationFn: async () => {
      ...
      if (form.kode) return updateKaryawan({ data: { kode: form.kode, ...data, aktif: form.aktif } });
    ...
    >
        ...
        <div className="kartu-farm divide-y-2 divide-dashed divide-border overflow-hidden">
          {karyawan.map((k: Karyawan) => (
            <div key={k.kode} className="flex items-center gap-3 p-4">
              ...
              <button
                onClick={() => {
                  if (confirm(`Hapus data karyawan "${k.nama}"? Riwayat absensinya juga ikut terhapus.`)) {
      ...
      {form ? (
        ...
        >
          ...
          >
            <p className="font-display text-2xl font-black">{form.kode ? "Edit Karyawan" : "Tambah Karyawan"}</p>

            <label className="mt-4 block text-sm font-bold text-muted-foreground">Nama</label>
            <input
              value={form.nama}
            ...
            <label className="mt-4 block text-sm font-bold text-muted-foreground">Gaji harian (Rp)</label>
            <input
              value={form.gajiHarian}
              onChange={(e) => setForm({ ...form, gajiHarian: e.target.value.replace(/\D/g, "") })}
              inputMode="numeric"
              placeholder="100000"
              className="mt-1 h-14 w-full rounded-xl border-2 border-border bg-background px-4 text-lg font-bold"
            />
            ...
            {pesanError ? <p className="mt-3 text-sm font-bold text-destructive">{pesanError}</p> : null}
